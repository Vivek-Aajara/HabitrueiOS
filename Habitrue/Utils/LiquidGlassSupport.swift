...
extension View {
    ...
    @ViewBuilder
    func appGlassCard(cornerRadius: CGFloat = AppTheme.cornerRadius, gradient: LinearGradient) -> some View {
        if #available(iOS 26.0, *) {
            self
                .background(gradient.opacity(0.55), in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
                .glassEffect(.regular, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
        } else {
            self.background(gradient, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
        }
    }
